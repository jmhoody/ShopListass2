...
class ShoppingUITableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "ShoppingUITableViewCell", for: indexPath)
        let item = shoppingItems[indexPath.row]
        
        if let itemCell = cell as? ShoppingUITableViewCell
        {
            itemCell.nameLabel.text = item.name
            itemCell.subtitleLabel.text = String(item.year)
        }
